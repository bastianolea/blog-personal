...
#
# Cambia `prueba <- TRUE` a FALSE para ejecutar los cambios reales.
...
prueba <- FALSE # TRUE = solo muestra qué haría; FALSE = descarga y modifica archivos
...
pdf_links <- map(md_files, function(file) {
  ...
  # si encuentra archivo, retorna tabla con ruta y link
  if (length(urls) == 0) {
    return(NULL)
  }
  tibble(
...
cat(sprintf(
  "Archivos con PDFs de WordPress: %d\n",
  n_distinct(pdf_table$file_path)
))
cat(sprintf(
  "URLs únicas encontradas:        %d\n",
...
discovery_path <- path(scripts_dir, "archivos_pdf.csv")
...
# por cada fila (par opost/archivo)
for (i in seq_len(nrow(download_results))) {
  ...
  if (prueba) {
    download_results$status[i] <- "prueba"
    download_results$message[i] <- "No descargado (prueba = TRUE)"
...
# Guardar log
log_path <- path(scripts_dir, "log_descargas_pdf.csv")
...
# en prueba también mostramos los reemplazos que se harían
pdfs_to_replace <- if (prueba) download_results else pdfs_ok
...
for (i in seq_len(nrow(files_to_update))) {
  ...
  if (prueba) {
    cat(sprintf(
      "[DRY RUN] Se reemplazarían %d enlace(s) en: %s\n",
      n_replaced,
      path_rel(fp, here::here())
    ))
  } else {
